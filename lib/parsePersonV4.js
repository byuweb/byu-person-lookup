...
function parseEmployeeSummaries (employeeSummaries) {
  ...
  const isActive = get(employeeSummaries, 'is_active.value')
  const activeStatus = isActive ? 'Active' : 'Not Active'
  return {
    employeeStatus: `${get(employeeSummaries, 'employee_or_contingent_worker_type.value')} (${activeStatus})`,
    department: get(employeeSummaries, 'supervisory_org.value'),
    jobTitle: get(employeeSummaries, 'business_title.value'),
    isActive: isActive
  }
}
