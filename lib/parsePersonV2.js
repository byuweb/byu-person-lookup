...
function parseBasic (basic) {
  const getPreferredName = basic => {
    const lastName = get(basic, 'preferred_surname.value', '')
    const firstName = get(basic, 'preferred_first_name.value', '')
    return `${lastName}, ${firstName}`
  }
  const name = get(basic, 'name_lnf.value', getPreferredName(basic)) || getPreferredName(basic)
...
function parseEmployeeSummaries (employeeSummaries) {
  ...
  return {
    employeeStatus: get(employeeSummaries, 'employee_type.value'),
