 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const{CustomEvent:CustomEvent$1}=window,executePersonsv4Request=async(e,t,n)=>{try{const{next:a,prev:r,people:s}=await search(e,n);return t.dispatchEvent(new CustomEvent$1("byu-lookup-datasource-result",{bubbles:!0,detail:s})),{next:a,prev:r}}catch(e){return console.error(e),t.dispatchEvent(new CustomEvent$1("byu-lookup-datasource-error",{bubbles:!0,detail:e})),{next:null,prev:null}}},setPendingSearch=e=>{const t=new CustomEvent$1("byu-lookup-datasource-searching",{bubbles:!0});e.dispatchEvent(t)};class ByuPersonsv4Datasource extends LitElement{connectedCallback(){super.connectedCallback(),connect();const e=new CustomEvent$1("byu-lookup-datasource-register",{bubbles:!0});this.dispatchEvent(e)}disconnectedCallback(){super.disconnectedCallback(),disconnect()}static get properties(){return{search:{type:String},next:{type:String},prev:{type:String}}}render(){this.search||(this.search="");const{label:e}=resolveSearchType(this.search);return html`${e}`}async performSearch(e){this.timeout&&clearTimeout(this.timeout),this.timeout=setTimeout(async()=>{setPendingSearch(this);const{next:t,prev:n}=await executePersonsv4Request(e,this);this.next=t,this.prev=n},100)}async nextPage(){this.timeout&&clearTimeout(this.timeout),this.timeout=setTimeout(async()=>{if(this.next){setPendingSearch(this);const{next:e,prev:t}=await executePersonsv4Request(this.search,this,this.next);this.next=e,this.prev=t}},100)}async prevPage(){this.timeout&&clearTimeout(this.timeout),this.timeout=setTimeout(async()=>{if(this.prev){setPendingSearch(this);const{next:e,prev:t}=await executePersonsv4Request(this.search,this,this.prev);this.next=e,this.prev=t}},100)}}console.log("registering personsv4 datasource"),window.customElements.define("byu-personsv4-datasource",ByuPersonsv4Datasource);
