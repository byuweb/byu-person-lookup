 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const pickFirst=(a,b)=>a||b;function parseAddresses(a){return 200===a.metadata.validation_response.code?a.values.reduce((a,b)=>{const c=[lodash_get(b,'address_line_1.value',''),lodash_get(b,'address_line_2.value',''),lodash_get(b,'address_line_3.value',''),lodash_get(b,'address_line_4.value','')].filter(a=>0<a.trim().length),d={MAL:'mailing',RES:'residential',WRK:'work',PRM:'permanent'}[b.address_type.value]||b.address_type.value;return Object.assign({},a,{[d]:c})},{}):null}function parseBasic(a){const b=lodash_get(a,'name_lnf.value',''),c=lodash_get(a,'byu_id.value',''),d=lodash_get(a,'net_id.value','');return{name:b,byuId:c,netId:d}}function parseEmailAddresses(a){return 200===a.metadata.validation_response.code?a.values.map(a=>lodash_get(a,'email_address.value','')).filter(a=>!!a).reduce(pickFirst,''):null}function parseEmployeeSummaries(a){return 200===a.metadata.validation_response.code?{employeeType:lodash_get(a,'employee_type.value'),department:lodash_get(a,'department.value'),jobTitle:lodash_get(a,'job_title.description')}:null}function parseStudentSummaries(a){if(200!==a.metadata.validation_response.code)return null;const b=lodash_get(a,'student_status.value');return{studentStatus:b}}function parsePersonV2(a){return Object.assign({addresses:parseAddresses(a.addresses),email:parseEmailAddresses(a.email_addresses)},parseBasic(a.basic),parseEmployeeSummaries(a.employee_summaries),parseStudentSummaries(a.student_summaries))}/*
 * Copyright 2018 Brigham Young University
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */let authHeader=null,observer=null;function connect(){observer||(observer=new AuthenticationObserver(({state:a,token:b,user:c,error:d})=>{authHeader=b?b.authorizationHeader:null}))}function nameParams(a){let b='',c='';if(/,/.test(a))[b,c]=a.split(',').map(a=>a.trim());else{// Assume 'First Middle Last'
const d=a.split(' ');b=d.slice(-1),c=d.slice(0,-1).join(' ')}return`?surname=${b}&rest_of_name=${c}`}function resolveSearchType(a){return 1>a.length?{q:`?surname=${a}`,label:'Search'}:/^\d{3,9}$/.test(a)?{q:`?byu_id=${a}`,label:'BYU ID'}:/^[a-z][a-z0-9]{2,7}$/.test(a)?{q:`?net_id=${a}`,label:'Net ID'}:/^[^@]+@.+$/.test(a)?{q:`?email_address=${a}`,label:'Email'}:/^[^ ]+ +[^ ]+[^0-9]*$/.test(a)?{q:`${nameParams(a)}`,label:'Name'}:{q:`?surname=${a}`,label:'Name'}}async function search(a){if(!authHeader)throw new Error('Not authenticated!');const{q:b}=resolveSearchType(a),c={method:'GET',headers:new window.Headers({Authorization:authHeader})},d=await window.fetch(`${'https://api.byu.edu:443/byuapi/persons/v2/'}${b}&field_sets=${'basic,addresses,email_addresses,employee_summaries,student_summaries'}&page_start=1&page_size=25`,c);//TODO: Support pagination
if(d.ok){const a=await d.json();return a.values.map(parsePersonV2)}if(404===d.status)return null;const e=`Error ${d.status} while querying personsv2`;throw new Error(e)}// in disconnectedCallback():
function disconnect(){observer&&observer.disconnect(),observer=null}export{connect,resolveSearchType,search,disconnect};
