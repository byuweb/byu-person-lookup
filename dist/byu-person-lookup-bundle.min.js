...
console.log("lookup-results::observing!",t),n.observe(t),this.isObserving=!0}}render(){// console.log(`byu-person-lookup-results::_render::searchPending=${searchPending}`)
    ...
    `,u=e=>html`
      <div class="card" @click=${()=>this.select(e)}>
        ...
        <div class="contact">
          <div>${c(o)}${e.email}</div>
          <div>${c(d)}${e.phone}</div>
          <div>${c(i)}${e.addresses?e.addresses.mailing?n(e.addresses.mailing):"":""}</div>
        </div>
      </div>
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class ByuPersonLookup extends LitElement{static get properties(){return{context:{type:String,reflect:!0},compact:{type:Boolean},errorMessage:{type:String},errorType:{type:String},results:{type:Array,attribute:!1},search:{type:String},searchPending:{type:Boolean}}}static get styles(){return[css` :host { display: inline-block; } :host([hidden]) { display: none; } `,css` * { font-family: 'HCo Ringside Narrow SSm', Arial Narrow, Arial, sans-serif; } `,css` div { position: relative; padding: 1rem; } `,css` .small-padding { padding: 0.25rem; } `,css` label { position: absolute; left: 1rem; top: -0.1rem; font-size: 0.7rem; color: #999; } `,css` input[type="search"] { padding: 0.3rem; border: thin solid #666666; border-radius: 0.2rem; margin-right: 0.2rem; min-width: 15rem; font-size: 1.1rem; } `,css` button { padding: 0.3rem 1rem; border: thin solid #666666; border-radius: 0.05rem; background-color: #0057B8; color: white; cursor: pointer; } `,css` button:hover, button:active { box-shadow: inset 0 0 0.2rem rgba(255, 255, 255, 0.5); background-color: #5199E1; } `,css` .spin { animation: spin 1500ms linear infinite; } `,css` .container { position: relative; font-size: 1.1rem; } `,css` .hidden { display: none; } `,css` .error-display { background-color: rgba(179, 4, 26, 0.8); color: white; position: absolute; top: 3.7rem; box-shadow: 0rem 0.1rem 0.1rem rgba(0, 0, 0, 0.2); } `,css` .error-handle { position: absolute; top: -10px; } `,css` @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } } .search-btn-label { display: none; } `,css` .compact { display: flex; padding: 0; }`,css` .compact > label { position: static; font-size: 0.9rem; align-self: center; } `,css` .compact > input[type="search"] { border-radius: 0; margin-right: 0; min-width: 10rem; font-size: 0.9rem; }`,css` .compact > button { border-radius: 0; }`,css` .compact > button .search-btn-label { display: none; } `,css` @media not speech { .sr-only { display: none; } } `,css` @media only screen and (min-width: 470px) { .search-btn-label { display: inline-block; } } `]}constructor(){super(),this.context="directory",this.compact=!1,this.errorMessage="",this.errorType="",this.results=null,this.search="",this.searchPending=!1}render(){const{context:e,results:t,search:n}=this,[,,,,r]=faSearch.icon,[,,,,s]=faSpinner.icon,[,,,,a]=faBan.icon,[,,,,o]=faExclamationTriangle.icon;// console.log(`search=${search}, context=${context}`)
return html`
