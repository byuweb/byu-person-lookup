 ...
 */class Template{constructor(e,t){this.parts=[],this.element=t;let n=-1,r=0;const s=[],o=t=>{const a=t.content,d=document.createTreeWalker(a,133/* NodeFilter.SHOW_{ELEMENT|COMMENT|TEXT} */,null,!1);// Edge needs all 4 parameters present; IE11 needs 3rd parameter to be
// null
...
let o=0;for(let e=0;e<s.length;e++)0<=s[e].value.indexOf(marker)&&o++;for(;0<o--;){// Get the template literal section leading up to the first
// expression in this attribute
const s=e.strings[r],o=lastAttributeNameRegex.exec(s)[2],a=o.toLowerCase()+boundAttributeSuffix,d=t.getAttribute(a),i=d.split(markerRegex);// Find the attribute name
this.parts.push({type:"attribute",index:n,name:o,strings:i}),t.removeAttribute(a),r+=i.length-1}}"TEMPLATE"===t.tagName&&o(t)}else if(3===t.nodeType/* Node.TEXT_NODE */){const e=t.data;if(0<=e.indexOf(marker)){const o=t.parentNode,a=e.split(markerRegex),d=a.length-1;// Generate a new text node for each literal section
// These nodes are also used as the markers for node parts
for(let e=0;e<d;e++)o.insertBefore(""===a[e]?createMarker():document.createTextNode(a[e]),t),this.parts.push({type:"node",index:++n});// If there's no text, we must insert a comment to mark our place.
// Else, we can trust it will stick around after cloning.
// We have a part for each match found
""===a[d]?(o.insertBefore(createMarker(),t),s.push(t)):t.data=a[d],r+=d}}else if(8===t.nodeType/* Node.COMMENT_NODE */)if(t.data===marker){const e=t.parentNode;// Add a new marker node to be the startNode of the Part if any of
// the following are true:
//  * We don't have a previousSibling
//  * The previousSibling is already the start of a previous part
(null===t.previousSibling||n===a)&&(n++,e.insertBefore(createMarker(),t)),a=n,this.parts.push({type:"node",index:n}),null===t.nextSibling?t.data="":(s.push(t),n--),r++}else for(let e=-1;-1!==(e=t.data.indexOf(marker,e+1));)// Comment node has a binding marker inside, make an inactive part
 ...
 */class DefaultTemplateProcessor{/**
     ...
     */handleAttributeExpressions(e,t,n,r){const s=t[0];if("."===s){const r=new PropertyCommitter(e,t.slice(1),n);return r.parts}if("@"===s)return[new EventPart(e,t.slice(1),r.eventContext)];if("?"===s)return[new BooleanAttributePart(e,t.slice(1),n)];const o=new AttributeCommitter(e,t,n);return o.parts}/**
 ...
 * removed 4 nodes)
 */function removeNodesFromTemplate(e,t){const{element:{content:r},parts:n}=e,s=document.createTreeWalker(r,walkerNodeFilter,null,!1);let o=nextActiveIndexInTemplateParts(n),a=n[o],d=-1,i=0;const l=[];for(let r=null;s.nextNode();){d++;const e=s.currentNode;// End removal if stepped past the removing node
...
if(null===n||void 0===n)return void s.appendChild(t);const o=document.createTreeWalker(s,walkerNodeFilter,null,!1);let a=nextActiveIndexInTemplateParts(r),d=0,i=-1;for(;o.nextNode();){i++;const e=o.currentNode;for(e===n&&(d=countNodes(t),n.parentNode.insertBefore(t,n));-1!==a&&r[a].index===i;){// If we've inserted the node, simply adjust all subsequent parts
if(0<d){for(;-1!==a;)r[a].index+=d,a=nextActiveIndexInTemplateParts(r,a);return}a=nextActiveIndexInTemplateParts(r,a)}}}/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 * http://polymer.github.io/LICENSE.txt
...
for(let o=0;o<r.length;o++){const e=r[o];e.parentNode.removeChild(e),s.textContent+=e.textContent}// Remove styles from nested templates in this scope.
...
const n=t.element.content.querySelector("style");e.insertBefore(n.cloneNode(!0),e.firstChild)}else{t.element.content.insertBefore(s,t.element.content.firstChild);const e=new Set;e.add(s),removeNodesFromTemplate(t,e)}},render$1=(e,t,n)=>{const r=n.scopeName,s=parts.has(t),o=t instanceof ShadowRoot&&compatibleShadyCSSVersion&&e instanceof TemplateResult,a=o&&!shadyRenderSet.has(r),d=a?document.createDocumentFragment():t;// When performing first scope render,
...
const e=()=>[1,2,3,4,5,6].map(()=>html`
    ...
    `,o=e=>e.showAdditionalInfo&&e.additionalInfo?html`
          <ul>
            ${e.additionalInfo.map(e=>html`<li>${e}</li>`)}
          </ul>
        `:html`<div></div>`,[,,,,a]=faEnvelope.icon,[,,,,d]=faPhone.icon,[,,,,i]=faHome.icon,[,,,,l]=faTimes.icon,p=svg`
 ...
 */class ByuPersonLookup extends LitElement{static get properties(){return{context:{type:String,reflect:!0},compact:{type:Boolean},noAutoselect:{type:Boolean,attribute:"no-autoselect"}}}static get styles(){return[css` :host { display: inline-block; } :host([hidden]) { display: none; } `,css` div { position: relative; padding: 1rem; } `,css` .small-padding { padding: 0.25rem; } `,css` label { position: absolute; left: 1rem; top: -0.1rem; font-size: 0.7rem; color: #999; } `,css` input[type="search"] {
      ...
      }`,css` .search-info { right: 0rem; }`,css` .search-info.position-to-right { transform-origin: top; left: -8rem; }`,css` .compact > .center-vertically > .search-info { top: 30px; }`,css` .search-info div { padding: 0; color: white; }`,css` .search-info-handle {
        position: absolute;
        top: -10px;
        right: 11px;
      }`,css` .position-to-right > .search-info-handle { left: calc(8rem + 11px); }`,css` .search-info-icon:hover > .search-info { transform: scale(1, 1); opacity: 1; }`,css` .hidden { display: none; }`]}constructor(){super(),this.context="directory",this.compact=!1,this.noAutoselect=!1,this.errorMessage="",this.errorType="",this.results=null,this.search="",this.searchPending=!1,this.hasSearchInfoSlotted=!1}render(){const{context:e,results:t,search:n}=this,[,,,,r]=faSearch.icon,[,,,,s]=faSpinner.icon,[,,,,o]=faBan.icon,[,,,,a]=faExclamationTriangle.icon,[,,,,d]=faQuestionCircle.icon,i=this.getBoundingClientRect();// console.log(`search=${search}, context=${context}`)
console.log("Bounding Rect:\n",i);const l=i.left<i.width,p=html`
...
this.prevPageFromProvider.call(this.__lookupProvider)}}console.log("registering person lookup"),window.customElements.define("byu-person-lookup",ByuPersonLookup),console.log("registering person lookup results"),window.customElements.define("byu-person-lookup-results",ByuPersonLookupResults);
