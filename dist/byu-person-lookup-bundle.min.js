...
`,r=e=>html$1`
    ...
    `,i=e=>/ACT/.test(e.employeeType)?html$1`
          <div>
            <div>${e.jobTitle}</div>
            <div>${e.department}</div>
            <div>${e.addresses.work?r(e.addresses.work):''}</div>
          </div>
    ...
    `,_=e=>html$1`
      <div class="card" on-click="${()=>this.select(e)}">
        ...
        <div class="contact">
          <div>${l(s)}${e.email}</div>
          <div>${l(d)}${e.phone}</div>
          <div>${l(o)}${e.addresses.mailing?r(e.addresses.mailing):''}</div>
        </div>
      </div>
    `;if(!e||1>e.length)return html$1``;const[,,,,p]=faTimes.icon,c=svg$1`
    ...
    `}select(e){const{personId:t,byuId:n,netId:r,name:a}=e,i=new CustomEvent('byu-lookup-results-select',{detail:{personId:t,byuId:n,netId:r,name:a},bubbles:!0,composed:!0});this.dispatchEvent(i),this.dispatchEvent(new CustomEvent('byu-lookup-results-close'))}close(){this.dispatchEvent(new CustomEvent('byu-lookup-results-close'))}}console.log('registering person lookup results'),window.customElements.define('byu-person-lookup-results',ByuPersonLookupResults);/*
 * Copyright 2018 Brigham Young University
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
