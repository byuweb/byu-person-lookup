 ...
 */const dedupingMixin=function(e){function t(t){let a=/** @type {!MixinFunction} */t.__mixinSet;if(a&&a[r])return t;let s=n,i=s.get(t);i||(i=/** @type {!Function} */e(t),s.set(t,i));// copy inherited mixin set from the extended class, or the base class
// NOTE: we avoid use of Set here because some browser (IE11)
// cannot extend a base Set via the constructor.
let d=Object.create(/** @type {!MixinFunction} */i.__mixinSet||a||null);return d[r]=!0,i.__mixinSet=d,i}let n=/** @type {!MixinFunction} */e.__mixinApplications;n||(n=new WeakMap,e.__mixinApplications=n);// maintain a unique id for each mixin
...
a=new TemplateInstance(r,e.partCallback,n),t.__templateInstance=a;const s=a._clone();a.update(e.values),removeNodes(t,t.firstChild),t.appendChild(s)}/**
...
const r=e.strings[s],i=lastAttributeNameRegex.exec(r)[1],d=n.getNamedItem(i),o=d.value.split(markerRegex);// Find the attribute name
// Find the corresponding attribute
// TODO(justinfagnani): remove non-null assertion
this.parts.push(new TemplatePart('attribute',a,d.name,i,o)),t.removeAttribute(d.name),s+=o.length-1}}else if(3===t.nodeType/* Node.TEXT_NODE */){const e=t.nodeValue;if(0>e.indexOf(marker))continue;const n=t.parentNode,r=e.split(markerRegex),d=r.length-1;// We have a part for each match found
s+=d;// Generate a new text node for each literal section
// These nodes are also used as the markers for node parts
for(let e=0;e<d;e++)n.insertBefore(''===r[e]?document.createComment(''):document.createTextNode(r[e]),t),this.parts.push(new TemplatePart('node',a++));n.insertBefore(''===r[d]?document.createComment(''):document.createTextNode(r[d]),t),i.push(t)}else if(8===t.nodeType/* Node.COMMENT_NODE */&&t.nodeValue===marker){const e=t.parentNode,r=t.previousSibling;// Add a new marker node to be the startNode of the Part if any of the
// following are true:
...
null===r||r!==n||r.nodeType!==Node.TEXT_NODE?e.insertBefore(document.createComment(''),t):a--,this.parts.push(new TemplatePart('node',a++)),i.push(t),null===t.nextSibling?e.insertBefore(document.createComment(''),t):a--,d=n,s++}}// Remove text binding nodes after the walk to not disturb the TreeWalker
 ...
 */class ByuPersonLookupResults extends LitElement{static get properties(){return{results:Array,context:String}}_render({results:e,context:t}){const n=html$1`
    ...
    `,s=e=>html$1`
      <tr on-click="${()=>this.select(e)}">
        <td>${e.name}</td>
        <td>${e.addresses.mailing?r(e.addresses.mailing):''}</td>
        <td>${e.email}</td>
      </tr>
    `;// TODO: address type hierarchy?
return!e||1>e.length?html$1``:html$1`
      ${n}
      <div class="modal">
        <div class="results">
    ...
    `}select(e){const{personId:t,byuId:n,netId:r,name:a}=e,s=new CustomEvent('byu-lookup-results-select',{detail:{personId:t,byuId:n,netId:r,name:a},bubbles:!0,composed:!0});this.dispatchEvent(s),this.dispatchEvent(new CustomEvent('byu-lookup-results-close'))}close(){this.dispatchEvent(new CustomEvent('byu-lookup-results-close'))}}console.log('registering person lookup results'),window.customElements.define('byu-person-lookup-results',ByuPersonLookupResults);/*
 * Copyright 2018 Brigham Young University
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class ByuPersonLookup extends LitElement{static get properties(){return{search:String,results:Object,searchPending:Boolean,context:String}}_render({search:e,results:t,searchPending:n,context:r}){console.log(`search=${e}, context=${r}`);const[,,,,a]=faSearch.icon,[,,,,s]=faSpinner.icon,i=html$1`
      <style>
        :host {
        }
        * {
    ...
    `}closeResults(){this.results=null}registerProvider(e){this.__lookupProvider=e,this.addEventListener('byu-lookup-datasource-result',this.searchResults),this.addEventListener('byu-lookup-datasource-error',this.searchError),this.fetchFromProvider=this.__lookupProvider.performSearch}connectedCallback(){super.connectedCallback();const e=this.firstElementChild;if(e&&e.performSearch)this.registerProvider(e);else{const t=setTimeout(()=>{throw new Error('No valid lookup provider found!')},1e4);this.addEventListener('byu-lookup-datasource-register',n=>{n.stopPropagation(),this.registerProvider(n.target),clearTimeout(t)})}}searchResults(t){t.stopPropagation(),console.log('search results:\n',t.detail),this.results=t.detail,this.searchPending=!1}searchError(t){t.stopPropagation(),console.log('search error:\n',t.detail)}searchChange(t){// console.log(`search=${this.search}`)
if(this.search=t.target.value,this.__lookupProvider){const e=this.search;this.__lookupProvider.search=e}}doSearch(){console.log(`doSearch:search: ${this.search}`),this.fetchFromProvider(this.search),this.searchPending=!0}}console.log('registering person lookup'),window.customElements.define('byu-person-lookup',ByuPersonLookup);export default ByuPersonLookup;
