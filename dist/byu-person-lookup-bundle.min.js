 ...
 */class Template{constructor(e,t){this.parts=[],this.element=t;let n=-1,r=0;const s=[],a=t=>{const o=t.content,d=document.createTreeWalker(o,133/* NodeFilter.SHOW_{ELEMENT|COMMENT|TEXT} */,null,!1);// Edge needs all 4 parameters present; IE11 needs 3rd parameter to be
// null
...
const s=e.strings[r],a=lastAttributeNameRegex.exec(s)[2],o=a.toLowerCase()+boundAttributeSuffix,d=t.getAttribute(o),i=d.split(markerRegex);// Find the attribute name
this.parts.push({type:"attribute",index:n,name:a,strings:i}),t.removeAttribute(o),r+=i.length-1}}"TEMPLATE"===t.tagName&&a(t)}else if(3===t.nodeType/* Node.TEXT_NODE */){const e=t.data;if(0<=e.indexOf(marker)){const a=t.parentNode,o=e.split(markerRegex),d=o.length-1;// Generate a new text node for each literal section
// These nodes are also used as the markers for node parts
for(let e=0;e<d;e++)a.insertBefore(""===o[e]?createMarker():document.createTextNode(o[e]),t),this.parts.push({type:"node",index:++n});// If there's no text, we must insert a comment to mark our place.
// Else, we can trust it will stick around after cloning.
// We have a part for each match found
""===o[d]?(a.insertBefore(createMarker(),t),s.push(t)):t.data=o[d],r+=d}}else if(8===t.nodeType/* Node.COMMENT_NODE */)if(t.data===marker){const e=t.parentNode;// Add a new marker node to be the startNode of the Part if any of
// the following are true:
//  * We don't have a previousSibling
//  * The previousSibling is already the start of a previous part
(null===t.previousSibling||n===o)&&(n++,e.insertBefore(createMarker(),t)),o=n,this.parts.push({type:"node",index:n}),null===t.nextSibling?t.data="":(s.push(t),n--),r++}else for(let e=-1;-1!==(e=t.data.indexOf(marker,e+1));)// Comment node has a binding marker inside, make an inactive part
...
// node if neccessasry.
if(!isTemplatePartActive(e))this._parts.push(void 0),n++;else if(r===e.index){if("node"===e.type){const e=this.processor.handleTextExpression(this.options);e.insertAfterNode(o.previousSibling),this._parts.push(e)}else this._parts.push(...this.processor.handleAttributeExpressions(o,e.name,e.strings,this.options));n++}else r++,"TEMPLATE"===o.nodeName&&s(o.content),o=a.nextNode()}};return s(e),isCEPolyfill&&(document.adoptNode(e),customElements.upgrade(e)),e}}/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 * http://polymer.github.io/LICENSE.txt
 ...
 * removed 4 nodes)
 */function removeNodesFromTemplate(e,t){const{element:{content:r},parts:n}=e,s=document.createTreeWalker(r,walkerNodeFilter,null,!1);let a=nextActiveIndexInTemplateParts(n),o=n[a],d=-1,i=0;const l=[];for(let r=null;s.nextNode();){d++;const e=s.currentNode;// End removal if stepped past the removing node
...
if(null===n||void 0===n)return void s.appendChild(t);const a=document.createTreeWalker(s,walkerNodeFilter,null,!1);let o=nextActiveIndexInTemplateParts(r),d=0,i=-1;for(;a.nextNode();){i++;const e=a.currentNode;for(e===n&&(d=countNodes(t),n.parentNode.insertBefore(t,n));-1!==o&&r[o].index===i;){// If we've inserted the node, simply adjust all subsequent parts
if(0<d){for(;-1!==o;)r[o].index+=d,o=nextActiveIndexInTemplateParts(r,o);return}o=nextActiveIndexInTemplateParts(r,o)}}}/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 * http://polymer.github.io/LICENSE.txt
...
const n=t.element.content.querySelector("style");e.insertBefore(n.cloneNode(!0),e.firstChild)}else{t.element.content.insertBefore(s,t.element.content.firstChild);const e=new Set;e.add(s),removeNodesFromTemplate(t,e)}},render$1=(e,t,n)=>{const r=n.scopeName,s=parts.has(t),a=t instanceof ShadowRoot&&compatibleShadyCSSVersion&&e instanceof TemplateResult,o=a&&!shadyRenderSet.has(r),d=o?document.createDocumentFragment():t;// When performing first scope render,
...
if(render(e,d,Object.assign({templateFactory:shadyTemplateFactory(r)},n)),o){const e=parts.get(d);parts.delete(d),e.value instanceof TemplateInstance&&prepareTemplateStyles(d,e.value.template,r),removeNodes(t,t.firstChild),t.appendChild(d),parts.set(t,e)}// After elements have hit the DOM, update styling if this is the
 ...
 */const{IntersectionObserver,CustomEvent}=window;class ByuPersonLookupResults extends LitElement{static get properties(){return{results:{type:Array},context:{type:String},noAutoselect:{type:Boolean},searchPending:{type:Boolean}}}static get styles(){return[css`
      :host {
        display: block;
      }
      :host[hidden] {
...
const e=this.shadowRoot.getElementById("top"),t=this.shadowRoot.getElementById("bottom");if(!IntersectionObserver||!e||!t)return;const n=new IntersectionObserver(n=>{n.forEach(n=>{n.isIntersecting&&(n.target===t?this.next():n.target===e&&this.prev())})},{});// observer.observe(top)
...
const e=()=>[1,2,3,4,5,6].map(()=>html`
    ...
    `,a=e=>e.showAdditionalInfo&&e.additionalInfo?html`
          <ul>
            ${e.additionalInfo.map(e=>html`<li>${e}</li>`)}
          </ul>
        `:html`<div></div>`,[,,,,o]=faEnvelope.icon,[,,,,d]=faPhone.icon,[,,,,i]=faHome.icon,[,,,,l]=faTimes.icon,p=svg`
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class ByuPersonLookup extends LitElement{static get properties(){return{context:{type:String,reflect:!0},compact:{type:Boolean},noAutoselect:{type:Boolean,attribute:"no-autoselect"}}}static get styles(){return[css` :host { display: inline-block; } :host([hidden]) { display: none; } `,css` div { position: relative; padding: 1rem; } `,css` .small-padding { padding: 0.25rem; } `,css` label { position: absolute; left: 1rem; top: -0.1rem; font-size: 0.7rem; color: #999; } `,css` input[type="search"] {
        padding: 0.3rem;
        border: thin solid #666666;
        ...
        cursor: pointer;
        font-size: 1.1rem;
      } `,css` button:hover, button:active { box-shadow: inset 0 0 0.2rem rgba(255, 255, 255, 0.5); background-color: #5199E1; } `,css` .spin { animation: spin 1500ms linear infinite; } `,css` .container { position: relative; font-size: 1.1rem; } `,css` .hidden { display: none; } `,css` .error-display {
        ...
        z-index: 19;
      } `,css` .error-handle { position: absolute; top: -10px; } `,css` @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } } .search-btn-label { display: none; } `,css` .compact { display: flex; padding: 0; }`,css` .compact > label { position: static; font-size: 0.9rem; align-self: center; margin: 0rem 0.2rem; } `,css` .compact > input[type="search"] { border-radius: 0; margin-right: 0; min-width: 10rem; font-size: 0.9rem; }`,css` .compact > button { border-radius: 0; font-size: 0.9rem; }`,css` .compact > button .search-btn-label { display: none; } `,css` .compact .error-display { top: 2.5rem; } `,css` @media not speech { .sr-only { display: none; } } `,css` @media only screen and (min-width: 470px) { .search-btn-label { display: inline-block; } } `]}constructor(){super(),this.context="directory",this.compact=!1,this.noAutoselect=!1,this.errorMessage="",this.errorType="",this.results=null,this.search="",this.searchPending=!1}render(){const{context:e,results:t,search:n}=this,[,,,,r]=faSearch.icon,[,,,,s]=faSpinner.icon,[,,,,a]=faBan.icon,[,,,,o]=faExclamationTriangle.icon;// console.log(`search=${search}, context=${context}`)
return html`
    <div class=${this.compact?"compact container":"container"}>
      ...
      <slot name="error">
        <div class=${""===this.errorType?"hidden":"error-display"}>
          ...
          ${"No Results"===this.errorType?this.errorMessage:html`
...
this.prevPageFromProvider()}}console.log("registering person lookup"),window.customElements.define("byu-person-lookup",ByuPersonLookup),console.log("registering person lookup results"),window.customElements.define("byu-person-lookup-results",ByuPersonLookupResults);
