...
for(let n,s;r.nextNode();){a++,n=s;const t=s=r.currentNode;if(1===t.nodeType/* Node.ELEMENT_NODE */){if(!t.hasAttributes())continue;const n=t.attributes;// Per https://developer.mozilla.org/en-US/docs/Web/API/NamedNodeMap,
...
this.parts.push(new TemplatePart('attribute',a,s.name,d,o)),t.removeAttribute(s.name),i+=o.length-1}}else if(3===t.nodeType/* Node.TEXT_NODE */){const e=t.nodeValue;if(0>e.indexOf(marker))continue;const n=t.parentNode,r=e.split(markerRegex),s=r.length-1;// We have a part for each match found
i+=s;// Generate a new text node for each literal section
// These nodes are also used as the markers for node parts
for(let e=0;e<s;e++)n.insertBefore(''===r[e]?document.createComment(''):document.createTextNode(r[e]),t),this.parts.push(new TemplatePart('node',a++));n.insertBefore(''===r[s]?document.createComment(''):document.createTextNode(r[s]),t),d.push(t)}else if(8===t.nodeType/* Node.COMMENT_NODE */&&t.nodeValue===marker){const e=t.parentNode,r=t.previousSibling;// Add a new marker node to be the startNode of the Part if any of the
// following are true:
...
null===r||r!==n||r.nodeType!==Node.TEXT_NODE?e.insertBefore(document.createComment(''),t):a--,this.parts.push(new TemplatePart('node',a++)),d.push(t),null===t.nextSibling?e.insertBefore(document.createComment(''),t):a--,s=n,i++}}// Remove text binding nodes after the walk to not disturb the TreeWalker
 ...
 */const getValue=(e,t)=>isDirective(t)?(t=t(e),noChange):null===t?void 0:t,isDirective=e=>'function'==typeof e&&!0===e.__litDirective,noChange={},isPrimitiveValue=e=>null===e||'object'!=typeof e&&'function'!=typeof e;/**
...
if(null===n||void 0===n)return void a.appendChild(t);const i=document.createTreeWalker(a,walkerNodeFilter,null,!1);for(let a=nextActiveIndexInTemplateParts(r),d=0,s=-1;i.nextNode();){s++;const e=i.currentNode;for(e===n&&(n.parentNode.insertBefore(t,n),d=countNodes(t));-1!==a&&r[a].index===s;){// If we've inserted the node, simply adjust all subsequent parts
...
`,a=()=>html$1`
    ...
    `),o=e=>/ACT|LEV/.test(e.employeeType)?html$1`
          <div>
            <div>${e.jobTitle}</div>
    ...
    `,u=e=>html$1`
      <div class="card" on-click="${()=>this.select(e)}">
        ...
        <div class="contact">
          <div>${c(l)}${e.email}</div>
          <div>${c(p)}${e.phone}</div>
          <div>${c(_)}${e.addresses?e.addresses.mailing?i(e.addresses.mailing):'':''}</div>
        </div>
      </div>
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class ByuPersonLookup extends LitElement{static get properties(){return{search:String,results:Object,searchPending:Boolean,context:String}}_render({search:e,results:t,searchPending:n,context:r}){console.log(`search=${e}, context=${r}`);const[,,,,a]=faSearch.icon,[,,,,i]=faSpinner.icon,d=html$1`
      <style>
        :host {
