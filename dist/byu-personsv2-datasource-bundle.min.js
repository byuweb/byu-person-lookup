 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const pickFirst=(e,t)=>e||t;function parseAddresses(e){return 200===e.metadata.validation_response.code?e.values.reduce((e,t)=>{const n=[lodash_get(t,'address_line_1.value',''),lodash_get(t,'address_line_2.value',''),lodash_get(t,'address_line_3.value',''),lodash_get(t,'address_line_4.value','')].filter(e=>0<e.trim().length),a={MAL:'mailing',RES:'residential',WRK:'work',PRM:'permanent'}[t.address_type.value]||t.address_type.value;return Object.assign({},e,{[a]:n})},{}):null}function parseBasic(e){const t=lodash_get(e,'name_lnf.value',''),n=lodash_get(e,'byu_id.value',''),a=lodash_get(e,'net_id.value','');return{name:t,byuId:n,netId:a}}function parseEmailAddresses(e){return 200===e.metadata.validation_response.code?e.values.map(t=>lodash_get(t,'email_address.value','')).filter(t=>!!t).reduce(pickFirst,''):null}function parseEmployeeSummaries(e){return 200===e.metadata.validation_response.code?{employeeType:lodash_get(e,'employee_type.value'),department:lodash_get(e,'department.value'),jobTitle:lodash_get(e,'job_title.description')}:null}function parseStudentSummaries(e){if(200!==e.metadata.validation_response.code)return null;const t=lodash_get(e,'student_status.value');return{studentStatus:t}}function parsePersonV2(e){return Object.assign({addresses:parseAddresses(e.addresses),email:parseEmailAddresses(e.email_addresses)},parseBasic(e.basic),parseEmployeeSummaries(e.employee_summaries),parseStudentSummaries(e.student_summaries))}/*
 * Copyright 2018 Brigham Young University
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
...
const a=e.split(' ');t=a.slice(-1),n=a.slice(0,-1).join(' ')}return`?surname=${t}&rest_of_name=${n}`}function resolveSearchType(e){return 1>e.length?{q:`?surname=${e}`,label:'Search'}:/^\d{3,9}$/.test(e)?{q:`?byu_id=${e}`,label:'BYU ID'}:/^[a-z][a-z0-9]{2,7}$/.test(e)?{q:`?net_id=${e}`,label:'Net ID'}:/^[^@]+@.+$/.test(e)?{q:`?email_address=${e}`,label:'Email'}:/^[^ ]+ +[^ ]+[^0-9]*$/.test(e)?{q:`${nameParams(e)}`,label:'Name'}:{q:`?surname=${e}`,label:'Name'}}async function search(e){if(!authHeader)throw new Error('Not authenticated!');const{q:t}=resolveSearchType(e),n={method:'GET',headers:new window.Headers({Authorization:authHeader})},a=await window.fetch(`${'https://api.byu.edu:443/byuapi/persons/v2/'}${t}&field_sets=${'basic,addresses,email_addresses,employee_summaries,student_summaries'}&page_start=1&page_size=25`,n);//TODO: Support pagination
if(a.ok){const e=await a.json();return e.values.map(parsePersonV2)}if(404===a.status)return null;const r=`Error ${a.status} while querying personsv2`;throw new Error(r)}// in disconnectedCallback():
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class ByuPersonsv2Datasource extends LitElement{connectedCallback(){super.connectedCallback(),connect();const e=new CustomEvent('byu-lookup-datasource-register',{bubbles:!0});this.dispatchEvent(e)}disconnectedCallback(){super.disconnectedCallback(),disconnect()}static get properties(){return{search:String}}_render({search:e}){e||(e='');const{label:t}=resolveSearchType(e);return html$1`${t}`}performSearch(e){this.timeout&&clearTimeout(this.timeout),this.timeout=setTimeout(()=>{search(e).then(e=>{this.dispatchEvent(new CustomEvent('byu-lookup-datasource-result',{bubbles:!0,detail:e}))}).catch(e=>{console.error(e),this.dispatchEvent(new CustomEvent('byu-lookup-datasource-error',{bubbles:!0,detail:e}))})},100)}}console.log('registering personsv2 datasource'),window.customElements.define('byu-personsv2-datasource',ByuPersonsv2Datasource);
