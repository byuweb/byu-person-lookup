(function(){'use strict';/**
     ...
     */function e(e){let t=I.get(e.type);void 0===t&&(t={stringsArray:new WeakMap,keyString:new Map},I.set(e.type,t));let n=t.stringsArray.get(e.strings);if(void 0!==n)return n;// If the TemplateStringsArray is new, generate a key from the strings
...
return n=t.keyString.get(r),void 0===n&&(n=new g(e,e.getTemplateElement()),t.keyString.set(r,n)),t.stringsArray.set(e.strings,n),n}/**
     ...
     * removed 4 nodes)
     */function t(e,t){const{element:{content:r},parts:n}=e,s=document.createTreeWalker(r,O,null,!1);let a=q(n),o=n[a],d=-1,i=0;const l=[];for(let r=null;s.nextNode();){d++;const e=s.currentNode;// End removal if stepped past the removing node
for(e.previousSibling===r&&(r=null),t.has(e)&&(l.push(e),null===r&&(r=e)),null!==r&&i++;o!==void 0&&o.index===d;)// If part is in a removed node deactivate it by setting index to -1 or
// adjust the index as needed.
// go to the next active part.
o.index=null===r?o.index-i:-1,a=q(n,a),o=n[a]}l.forEach(e=>e.parentNode.removeChild(e))}/**
...
if(null===n||void 0===n)return void s.appendChild(t);const a=document.createTreeWalker(s,O,null,!1);let o=q(r),d=0,i=-1;for(;a.nextNode();){i++;const e=a.currentNode;for(e===n&&(d=U(t),n.parentNode.insertBefore(t,n));-1!==o&&r[o].index===i;){// If we've inserted the node, simply adjust all subsequent parts
if(0<d){for(;-1!==o;)r[o].index+=d,o=q(r,o);return}o=q(r,o)}}}/**
     * @license
     * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
     * This code may only be used under the BSD style license found at
     * http://polymer.github.io/LICENSE.txt
...
const r=new WeakMap,s=e=>"function"==typeof e&&r.has(e),a=window.customElements!==void 0&&window.customElements.polyfillWrapFlushCallback!==void 0,o=(e,t,n=null,r=null)=>{for(let s=t;s!==n;){const t=s.nextSibling;e.insertBefore(s,r),s=t}},d=(e,t,n=null)=>{for(let r=t;r!==n;){const t=r.nextSibling;e.removeChild(r),r=t}},i={},l={},p=`{{lit-${(Math.random()+"").slice(2)}}}`,u=`<!--${p}-->`,c=new RegExp(`${p}|${u}`),m="$lit$";/**
     * An updateable Template that tracks the location of dynamic parts.
     */class g{constructor(e,t){this.parts=[],this.element=t;let n=-1,r=0;const s=[],a=t=>{const o=t.content,d=document.createTreeWalker(o,133/* NodeFilter.SHOW_{ELEMENT|COMMENT|TEXT} */,null,!1);// Edge needs all 4 parameters present; IE11 needs 3rd parameter to be
// null
for(// Keeps track of the last index associated with a part. We try to delete
// unnecessary nodes, but we never want to associate two different parts
// to the same index. They must have a constant node between.
let o=0;d.nextNode();){n++;const t=d.currentNode;if(1===t.nodeType/* Node.ELEMENT_NODE */){if(t.hasAttributes()){const s=t.attributes;// Per
// https://developer.mozilla.org/en-US/docs/Web/API/NamedNodeMap,
// attributes are not guaranteed to be returned in document order.
// In particular, Edge/IE can return them out of order, so we cannot
// assume a correspondance between part index and attribute index.
let a=0;for(let e=0;e<s.length;e++)0<=s[e].value.indexOf(p)&&a++;for(;0<a--;){// Get the template literal section leading up to the first
// expression in this attribute
const s=e.strings[r],a=y.exec(s)[2],o=a.toLowerCase()+m,d=t.getAttribute(o),i=d.split(c);// Find the attribute name
this.parts.push({type:"attribute",index:n,name:a,strings:i}),t.removeAttribute(o),r+=i.length-1}}"TEMPLATE"===t.tagName&&a(t)}else if(3===t.nodeType/* Node.TEXT_NODE */){const e=t.data;if(0<=e.indexOf(p)){const a=t.parentNode,o=e.split(c),d=o.length-1;// Generate a new text node for each literal section
// These nodes are also used as the markers for node parts
for(let e=0;e<d;e++)a.insertBefore(""===o[e]?h():document.createTextNode(o[e]),t),this.parts.push({type:"node",index:++n});// If there's no text, we must insert a comment to mark our place.
// Else, we can trust it will stick around after cloning.
// We have a part for each match found
""===o[d]?(a.insertBefore(h(),t),s.push(t)):t.data=o[d],r+=d}}else if(8===t.nodeType/* Node.COMMENT_NODE */)if(t.data===p){const e=t.parentNode;// Add a new marker node to be the startNode of the Part if any of
// the following are true:
//  * We don't have a previousSibling
//  * The previousSibling is already the start of a previous part
(null===t.previousSibling||n===o)&&(n++,e.insertBefore(h(),t)),o=n,this.parts.push({type:"node",index:n}),null===t.nextSibling?t.data="":(s.push(t),n--),r++}else for(let e=-1;-1!==(e=t.data.indexOf(p,e+1));)// Comment node has a binding marker inside, make an inactive part
...
for(const r of s)r.parentNode.removeChild(r)}}const f=e=>-1!==e.index,h=()=>document.createComment(""),y=/([ \x09\x0a\x0c\x0d])([^\0-\x1F\x7F-\x9F \x09\x0a\x0c\x0d"'>=/]+)([ \x09\x0a\x0c\x0d]*=[ \x09\x0a\x0c\x0d]*(?:[^ \x09\x0a\x0c\x0d"'`<>=]*|"[^"]*|'[^']*))$/;// Allows `document.createComment('')` to be renamed for a
...
const e=a?this.template.element.content.cloneNode(!0):document.importNode(this.template.element.content,!0),t=this.template.parts;let n=0,r=0;const s=e=>{// Edge needs all 4 parameters present; IE11 needs 3rd parameter to be
// null
...
for(let o=a.nextNode();n<t.length&&null!==o;){const e=t[n];// Consecutive Parts may have the same node index, in the case of
// multiple bound attributes on an element. So each iteration we either
// increment the nodeIndex, if we aren't on a node with a part, or the
// partIndex if we are. By not incrementing the nodeIndex when we find a
// part, we allow for the next part to be associated with the current
// node if neccessasry.
if(!f(e))this._parts.push(void 0),n++;else if(r===e.index){if("node"===e.type){const e=this.processor.handleTextExpression(this.options);e.insertAfterNode(o.previousSibling),this._parts.push(e)}else this._parts.push(...this.processor.handleAttributeExpressions(o,e.name,e.strings,this.options));n++}else r++,"TEMPLATE"===o.nodeName&&s(o.content),o=a.nextNode()}};return s(e),a&&(document.adoptNode(e),customElements.upgrade(e)),e}}/**
     * @license
     * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
     * This code may only be used under the BSD style license found at
     * http://polymer.github.io/LICENSE.txt
...
t+=r?e.substr(0,r.index)+r[1]+r[2]+m+r[3]+p:e+u}return t+this.strings[e]}getTemplateElement(){const e=document.createElement("template");return e.innerHTML=this.getHTML(),e}}/**
     ...
     */class S extends v{getHTML(){return`<svg>${super.getHTML()}</svg>`}getTemplateElement(){const e=super.getTemplateElement(),t=e.content,n=t.firstChild;return t.removeChild(n),o(t,n.firstChild),e}}/**
     * @license
     * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
     * This code may only be used under the BSD style license found at
     * http://polymer.github.io/LICENSE.txt
     ...
     */class b{constructor(e,t,n){this.dirty=!0,this.element=e,this.name=t,this.strings=n,this.parts=[];for(let r=0;r<n.length-1;r++)this.parts[r]=this._createPart()}/**
         * Creates a single part. Override this to create a differnt type of part.
         */_createPart(){return new x(this)}_getValue(){const e=this.strings,t=e.length-1;let n="";for(let r=0;r<t;r++){n+=e[r];const t=this.parts[r];if(void 0!==t){const e=t.value;if(null!=e&&(Array.isArray(e)||// tslint:disable-next-line:no-any
"string"!=typeof e&&e[Symbol.iterator]))for(const r of e)n+="string"==typeof r?r:r+"";else n+="string"==typeof e?e:e+""}}return n+=e[t],n}commit(){this.dirty&&(this.dirty=!1,this.element.setAttribute(this.name,this._getValue()))}}class x{constructor(e){this.value=void 0,this.committer=e}setValue(e){e===i||P(e)&&e===this.value||(this.value=e,!s(e)&&(this.committer.dirty=!0))}commit(){for(;s(this.value);){const e=this.value;this.value=i,e(this)}this.value===i||this.committer.commit()}}class N{constructor(e){this.value=void 0,this._pendingValue=void 0,this.options=e}/**
         * Inserts this part into a container.
         *
         * This part must be empty, as its contents are not automatically moved.
         */appendInto(e){this.startNode=e.appendChild(h()),this.endNode=e.appendChild(h())}/**
         ...
         *
         * This part must be empty, as its contents are not automatically moved.
         */appendIntoPart(e){e._insert(this.startNode=h()),e._insert(this.endNode=h())}/**
         * Appends this part after `ref`
         *
         * This part must be empty, as its contents are not automatically moved.
         */insertAfterPart(e){e._insert(this.startNode=h()),this.endNode=e.endNode,e.endNode=this.startNode}setValue(e){this._pendingValue=e}commit(){for(;s(this._pendingValue);){const e=this._pendingValue;this._pendingValue=i,e(this)}const e=this._pendingValue;e===i||(P(e)?e!==this.value&&this._commitText(e):e instanceof v?this._commitTemplateResult(e):e instanceof Node?this._commitNode(e):Array.isArray(e)||// tslint:disable-next-line:no-any
...
const k=e=>e&&(A?{capture:e.capture,passive:e.passive,once:e.once}:e.capture);/**
     ...
     */const z=new class{/**
         ...
         */handleAttributeExpressions(e,t,n,r){const s=t[0];if("."===s){const r=new T(e,t.slice(1),n);return r.parts}if("@"===s)return[new V(e,t.slice(1),r.eventContext)];if("?"===s)return[new C(e,t.slice(1),n)];const a=new b(e,t,n);return a.parts}/**
         * Create parts for a text-position binding.
         * @param templateFactory
         */handleTextExpression(e){return new N(e)}},I=new Map,M=new WeakMap,L=(t,n,r)=>{let s=M.get(n);s===void 0&&(d(n,n.firstChild),M.set(n,s=new N(Object.assign({templateFactory:e},r))),s.appendInto(n)),s.setValue(t),s.commit()};(window.litHtmlVersions||(window.litHtmlVersions=[])).push("1.0.0");/**
     * Interprets a template literal as an HTML template that can efficiently
     * render to and update a container.
     */const R=(e,...t)=>new v(e,t,"html",z),w=(e,...t)=>new S(e,t,"svg",z),O=133/* NodeFilter.SHOW_{ELEMENT|COMMENT|TEXT} */,U=e=>{let t=11===e.nodeType/* Node.DOCUMENT_FRAGMENT_NODE */?0:1;for(const n=document.createTreeWalker(e,O,null,!1);n.nextNode();)t++;return t},q=(e,t=-1)=>{for(let n=t+1;n<e.length;n++){const t=e[n];if(f(t))return n}return-1},H=(e,t)=>`${e}--${t}`;/**
     * Interprets a template literal as an SVG template that can efficiently
     * render to and update a container.
     */let F=!0;"undefined"==typeof window.ShadyCSS?F=!1:"undefined"==typeof window.ShadyCSS.prepareTemplateDom&&(console.warn("Incompatible ShadyCSS version detected.Please update to at least @webcomponents/webcomponentsjs@2.0.2 and@webcomponents/shadycss@1.3.1."),F=!1);/**
     * Template factory which scopes template DOM using ShadyCSS.
     * @param scopeName {string}
     */const B=e=>t=>{const n=H(t.type,e);let r=I.get(n);void 0===r&&(r={stringsArray:new WeakMap,keyString:new Map},I.set(n,r));let s=r.stringsArray.get(t.strings);if(void 0!==s)return s;const a=t.strings.join(p);if(s=r.keyString.get(a),void 0===s){const n=t.getTemplateElement();F&&window.ShadyCSS.prepareTemplateDom(n,e),s=new g(t,n),r.keyString.set(a,s)}return r.stringsArray.set(t.strings,s),s},W=["html","svg"],D=e=>{W.forEach(n=>{const r=I.get(H(n,e));r!==void 0&&r.keyString.forEach(e=>{const{element:{content:n}}=e,r=new Set;// IE 11 doesn't support the iterable param Set constructor
Array.from(n.querySelectorAll("style")).forEach(e=>{r.add(e)}),t(e,r)})})},j=new Set,$=(e,r,s)=>{j.add(s);// Move styles out of rendered DOM and store.
const a=e.querySelectorAll("style");// If there are no styles, skip unnecessary work
if(0===a.length)return void window.ShadyCSS.prepareTemplateStyles(r.element,s);const o=document.createElement("style");// Collect styles into a single style. This helps us make sure ShadyCSS
// manipulations will not prevent us from being able to fix up template
// part indices.
// NOTE: collecting styles is inefficient for browsers but ShadyCSS
// currently does this anyway. When it does not, this should be changed.
for(let t=0;t<a.length;t++){const e=a[t];e.parentNode.removeChild(e),o.textContent+=e.textContent}// Remove styles from nested templates in this scope.
if(D(s),n(r,o,r.element.content.firstChild),window.ShadyCSS.prepareTemplateStyles(r.element,s),window.ShadyCSS.nativeShadow){// When in native Shadow DOM, re-add styling to rendered content using
// the style ShadyCSS produced.
const t=r.element.content.querySelector("style");e.insertBefore(t.cloneNode(!0),e.firstChild)}else{r.element.content.insertBefore(o,r.element.content.firstChild);const e=new Set;e.add(o),t(r,e)}},G=(e,t,n)=>{const r=n.scopeName,s=M.has(t),a=t instanceof ShadowRoot&&F&&e instanceof v,o=a&&!j.has(r),i=o?document.createDocumentFragment():t;// When performing first scope render,
...
if(L(e,i,Object.assign({templateFactory:B(r)},n)),o){const e=M.get(i);M.delete(i),e.value instanceof _&&$(i,e.value.template,r),d(t,t.firstChild),t.appendChild(i),M.set(t,e)}// After elements have hit the DOM, update styling if this is the
// initial render to this container.
// This is needed whenever dynamic changes are made so it would be
// safest to do every render; however, this would regress performance
// so we leave it up to the user to call `ShadyCSSS.styleElement`
// for dynamic changes.
!s&&a&&window.ShadyCSS.styleElement(t.host)},J=e=>e,X=(e,t)=>{if(e in t)for(;t!==Object.prototype;){if(t.hasOwnProperty(e))return Object.getOwnPropertyDescriptor(t,e);t=Object.getPrototypeOf(t)}},Y={toAttribute(e,t){return t===Boolean?e?"":null:t===Object||t===Array?null==e?e:JSON.stringify(e):e},fromAttribute(e,t){return t===Boolean?null!==e:t===Number?null===e?null:+e:t===Object||t===Array?JSON.parse(e):e}},Z=(e,t)=>t!==e&&(t===t||e===e),Q={attribute:!0,type:String,converter:Y,reflect:!1,hasChanged:Z},K=Promise.resolve(!0),ee=1,te=4,ne=8,re=16,se=32;/**
     * Base element class which manages element properties and attributes. When
     * properties change, the `update` method is asynchronously called. This method
     * should be supplied by subclassers to render updates as desired.
     */class ae extends HTMLElement{constructor(){/**
...
const e=Object.getPrototypeOf(this)._classProperties;e!==void 0&&e.forEach((e,t)=>this._classProperties.set(t,e))}}/**
         ...
         * an update.
         * @nocollapse
         */static createProperty(e,t=Q){if(this._ensureClassProperties(),this._classProperties.set(e,t),!t.noAccessor){const t=X(e,this.prototype);let n;// If there is a super accessor, capture it and "super" to it
...
this.createProperty(n,e[n])}}}/**
         ...
         * @nocollapse
         */static _propertyValueFromAttribute(e,t){const n=t.type,r=t.converter||Y,s="function"==typeof r?r:r.fromAttribute;return s?s(e,n):e}/**
         ...
         * @nocollapse
         */static _propertyValueToAttribute(e,t){if(void 0===t.reflect)return;const n=t.type,r=t.converter,s=r&&r.toAttribute||Y.toAttribute;return s(e,n)}/**
         ...
         */attributeChangedCallback(e,t,n){t!==n&&this._attributeToProperty(e,n)}_propertyToAttribute(e,t,n=Q){const r=this.constructor,s=r._attributeNameForProperty(e,n);if(s!==void 0){const e=r._propertyValueToAttribute(t,n);// an undefined value does not change the attribute.
...
if(this._updateState&ne)return;const n=this.constructor,r=n._attributeToPropertyMap.get(e);if(r!==void 0){const e=n._classProperties.get(r)||Q;// mark state reflecting
// mark state not reflecting
...
null!=n&&"function"==typeof n.then&&(await n),e(!this._hasRequestedUpdate)}get _hasConnected(){return this._updateState&se}get _hasRequestedUpdate(){return this._updateState&te}get hasUpdated(){return this._updateState&ee}/**
    ...
    */const oe="adoptedStyleSheets"in Document.prototype;class de{constructor(e){this.cssText=e}// Note, this is a getter so that it's lazy. In practice, this means
// stylesheets are not created until the first element instance is made.
get styleSheet(){return void 0===this._styleSheet&&(oe?(this._styleSheet=new CSSStyleSheet,this._styleSheet.replaceSync(this.cssText)):this._styleSheet=null),this._styleSheet}}const ie=e=>{if(e instanceof de)return e.cssText;throw new Error(`Value passed to 'css' function must be a 'css' function result: ${e}.`)},le=(e,...t)=>{const n=t.reduce((t,n,r)=>t+ie(n)+e[r+1],e[0]);return new de(n)};/**
     * @license
     * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
     * This code may only be used under the BSD style license found at
     * http://polymer.github.io/LICENSE.txt
...
// rendering
     ...
     */pe.finalized=!0,pe.render=G;var ue={prefix:"fas",iconName:"search",icon:[512,512,[],"f002","M505 442.7L405.3 343c-4.5-4.5-10.6-7-17-7H372c27.6-35.3 44-79.7 44-128C416 93.1 322.9 0 208 0S0 93.1 0 208s93.1 208 208 208c48.3 0 92.7-16.4 128-44v16.3c0 6.4 2.5 12.5 7 17l99.7 99.7c9.4 9.4 24.6 9.4 33.9 0l28.3-28.3c9.4-9.4 9.4-24.6.1-34zM208 336c-70.7 0-128-57.2-128-128 0-70.7 57.2-128 128-128 70.7 0 128 57.2 128 128 0 70.7-57.2 128-128 128z"]},ce={prefix:"fas",iconName:"spinner",icon:[512,512,[],"f110","M304 48c0 26.51-21.49 48-48 48s-48-21.49-48-48 21.49-48 48-48 48 21.49 48 48zm-48 368c-26.51 0-48 21.49-48 48s21.49 48 48 48 48-21.49 48-48-21.49-48-48-48zm208-208c-26.51 0-48 21.49-48 48s21.49 48 48 48 48-21.49 48-48-21.49-48-48-48zM96 256c0-26.51-21.49-48-48-48S0 229.49 0 256s21.49 48 48 48 48-21.49 48-48zm12.922 99.078c-26.51 0-48 21.49-48 48s21.49 48 48 48 48-21.49 48-48c0-26.509-21.491-48-48-48zm294.156 0c-26.51 0-48 21.49-48 48s21.49 48 48 48 48-21.49 48-48c0-26.509-21.49-48-48-48zM108.922 60.922c-26.51 0-48 21.49-48 48s21.49 48 48 48 48-21.49 48-48-21.491-48-48-48z"]},me={prefix:"fas",iconName:"ban",icon:[512,512,[],"f05e","M256 8C119.034 8 8 119.033 8 256s111.034 248 248 248 248-111.034 248-248S392.967 8 256 8zm130.108 117.892c65.448 65.448 70 165.481 20.677 235.637L150.47 105.216c70.204-49.356 170.226-44.735 235.638 20.676zM125.892 386.108c-65.448-65.448-70-165.481-20.677-235.637L361.53 406.784c-70.203 49.356-170.226 44.736-235.638-20.676z"]},ge={prefix:"fas",iconName:"exclamation-triangle",icon:[576,512,[],"f071","M569.517 440.013C587.975 472.007 564.806 512 527.94 512H48.054c-36.937 0-59.999-40.055-41.577-71.987L246.423 23.985c18.467-32.009 64.72-31.951 83.154 0l239.94 416.028zM288 354c-25.405 0-46 20.595-46 46s20.595 46 46 46 46-20.595 46-46-20.595-46-46-46zm-43.673-165.346l7.418 136c.347 6.364 5.609 11.346 11.982 11.346h48.546c6.373 0 11.635-4.982 11.982-11.346l7.418-136c.375-6.874-5.098-12.654-11.982-12.654h-63.383c-6.884 0-12.356 5.78-11.981 12.654z"]},fe={prefix:"fas",iconName:"times",icon:[352,512,[],"f00d","M242.72 256l100.07-100.07c12.28-12.28 12.28-32.19 0-44.48l-22.24-22.24c-12.28-12.28-32.19-12.28-44.48 0L176 189.28 75.93 89.21c-12.28-12.28-32.19-12.28-44.48 0L9.21 111.45c-12.28 12.28-12.28 32.19 0 44.48L109.28 256 9.21 356.07c-12.28 12.28-12.28 32.19 0 44.48l22.24 22.24c12.28 12.28 32.2 12.28 44.48 0L176 322.72l100.07 100.07c12.28 12.28 32.2 12.28 44.48 0l22.24-22.24c12.28-12.28 12.28-32.19 0-44.48L242.72 256z"]},he={prefix:"fas",iconName:"envelope",icon:[512,512,[],"f0e0","M502.3 190.8c3.9-3.1 9.7-.2 9.7 4.7V400c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V195.6c0-5 5.7-7.8 9.7-4.7 22.4 17.4 52.1 39.5 154.1 113.6 21.1 15.4 56.7 47.8 92.2 47.6 35.7.3 72-32.8 92.3-47.6 102-74.1 131.6-96.3 154-113.7zM256 320c23.2.4 56.6-29.2 73.4-41.4 132.7-96.3 142.8-104.7 173.4-128.7 5.8-4.5 9.2-11.5 9.2-18.9v-19c0-26.5-21.5-48-48-48H48C21.5 64 0 85.5 0 112v19c0 7.4 3.4 14.3 9.2 18.9 30.6 23.9 40.7 32.4 173.4 128.7 16.8 12.2 50.2 41.8 73.4 41.4z"]},ye={prefix:"fas",iconName:"phone",icon:[512,512,[],"f095","M493.4 24.6l-104-24c-11.3-2.6-22.9 3.3-27.5 13.9l-48 112c-4.2 9.8-1.4 21.3 6.9 28l60.6 49.6c-36 76.7-98.9 140.5-177.2 177.2l-49.6-60.6c-6.8-8.3-18.2-11.1-28-6.9l-112 48C3.9 366.5-2 378.1.6 389.4l24 104C27.1 504.2 36.7 512 48 512c256.1 0 464-207.5 464-464 0-11.2-7.7-20.9-18.6-23.4z"]},_e={prefix:"fas",iconName:"home",icon:[576,512,[],"f015","M488 312.7V456c0 13.3-10.7 24-24 24H348c-6.6 0-12-5.4-12-12V356c0-6.6-5.4-12-12-12h-72c-6.6 0-12 5.4-12 12v112c0 6.6-5.4 12-12 12H112c-13.3 0-24-10.7-24-24V312.7c0-3.6 1.6-7 4.4-9.3l188-154.8c4.4-3.6 10.8-3.6 15.3 0l188 154.8c2.7 2.3 4.3 5.7 4.3 9.3zm83.6-60.9L488 182.9V44.4c0-6.6-5.4-12-12-12h-56c-6.6 0-12 5.4-12 12V117l-89.5-73.7c-17.7-14.6-43.3-14.6-61 0L4.4 251.8c-5.1 4.2-5.8 11.8-1.6 16.9l25.5 31c4.2 5.1 11.8 5.8 16.9 1.6l235.2-193.7c4.4-3.6 10.8-3.6 15.3 0l235.2 193.7c5.1 4.2 12.7 3.5 16.9-1.6l25.5-31c4.2-5.2 3.4-12.7-1.7-16.9z"]};/*
     ...
     */const{IntersectionObserver:ve,CustomEvent:Se}=window;class Pe extends pe{static get properties(){return{results:{type:Array},context:{type:String},noAutoselect:{type:Boolean},searchPending:{type:Boolean}}}static get styles(){return[le`
      :host {
        display: block;
      }
      :host[hidden] {
...
const e=this.shadowRoot.getElementById("top"),t=this.shadowRoot.getElementById("bottom");if(!ve||!e||!t)return;const n=new ve(n=>{n.forEach(n=>{n.isIntersecting&&(n.target===t?this.next():n.target===e&&this.prev())})},{});// observer.observe(top)
...
const e=()=>[1,2,3,4,5,6].map(()=>R`
    ...
    `,a=e=>e.showAdditionalInfo&&e.additionalInfo?R`
          <ul>
            ${e.additionalInfo.map(e=>R`<li>${e}</li>`)}
          </ul>
        `:R`<div></div>`,[,,,,o]=he.icon,[,,,,d]=ye.icon,[,,,,i]=_e.icon,[,,,,l]=fe.icon,p=w`
    <path d=${l} fill="white" transform="translate(90)"/>
    `,u=e=>R`
      <svg width="14" height="14" viewBox="0 0 512 512">
        ${w`<path d=${e} fill="#666666"/>`}
      </svg>
    `,c=e=>R`
    ...
    `;return this.results&&this.results.map&&!(1>this.results.length)?R`
      <div class="modal">
        <div class="results">
          <h2 id="top">Lookup Results</h2>
          ${this.context&&"admin"===this.context?(t=>R`
      <table>
        <thead>
          <tr>
            <th>Name</th>
    ...
    `)(this.results)}
          ...
          ${ve?R`<button id="bottom" class="nav-btn" @click=${this.close}>Close</button>`:R`<div>
     ...
     */class be extends pe{static get properties(){return{context:{type:String,reflect:!0},compact:{type:Boolean},noAutoselect:{type:Boolean,attribute:"no-autoselect"}}}static get styles(){return[le` :host { display: inline-block; } :host([hidden]) { display: none; } `,le` div { position: relative; padding: 1rem; } `,le` .small-padding { padding: 0.25rem; } `,le` label { position: absolute; left: 1rem; top: -0.1rem; font-size: 0.7rem; color: #999; } `,le` input[type="search"] {
      ...
      } `,le` .error-handle { position: absolute; top: -10px; } `,le` @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } } .search-btn-label { display: none; } `,le` .compact { display: flex; padding: 0; }`,le` .compact > label { position: static; font-size: 0.9rem; align-self: center; margin: 0rem 0.2rem; } `,le` .compact > input[type="search"] { border-radius: 0; margin-right: 0; min-width: 10rem; font-size: 0.9rem; }`,le` .compact > button { border-radius: 0; font-size: 0.9rem; }`,le` .compact > button .search-btn-label { display: none; } `,le` @media not speech { .sr-only { display: none; } } `,le` @media only screen and (min-width: 470px) { .search-btn-label { display: inline-block; } } `]}constructor(){super(),this.context="directory",this.compact=!1,this.noAutoselect=!1,this.errorMessage="",this.errorType="",this.results=null,this.search="",this.searchPending=!1}render(){const{context:e,results:t,search:n}=this,[,,,,r]=ue.icon,[,,,,s]=ce.icon,[,,,,a]=me.icon,[,,,,o]=ge.icon;// console.log(`search=${search}, context=${context}`)
return R`
    <div class=${this.compact?"compact container":"container"}>
      ...
      <slot name="error">
        <div class=${""===this.errorType?"hidden":"error-display"}>
          ...
          ${w`
            <path
              d=${"No Results"===this.errorType?a:o}
              fill="white"
            />
          `}
          </svg>
          ${"No Results"===this.errorType?this.errorMessage:R`
    ...
    `}closeResults(){this.results=null,this.requestUpdate()}registerProvider(e){this.__lookupProvider=e,this.addEventListener("byu-lookup-datasource-result",this.searchResults),this.addEventListener("byu-lookup-datasource-error",this.searchError),this.addEventListener("byu-lookup-datasource-searching",this.searchBegun),this.fetchFromProvider=this.__lookupProvider.performSearch,this.nextPageFromProvider=this.__lookupProvider.nextPage,this.prevPageFromProvider=this.__lookupProvider.prevPage}connectedCallback(){super.connectedCallback();const e=this.firstElementChild;if(e&&e.performSearch)this.registerProvider(e);else{const t=setTimeout(()=>{throw new Error("No valid lookup provider found!")},1e4);this.addEventListener("byu-lookup-datasource-register",n=>{n.stopPropagation(),this.registerProvider(n.target),clearTimeout(t)})}}searchResults(t){return t.stopPropagation(),this.searchPending=!1,0===t.detail.length&&0===this.results.length?(this.errorMessage="Hmmm, we couldn't find anyone.",this.errorType="No Results",void this.requestUpdate()):void(this.results=this.results.concat(t.detail),this.requestUpdate())}searchError(t){// Don't trigger any other lookup components
// window.alert(e.detail)
t.stopPropagation(),this.searchPending=!1,this.errorMessage=t.detail,this.errorType="Service Error",console.error("search error:\n",t.detail),this.requestUpdate()}searchBegun(t){// Don't trigger any other lookup components
t.stopPropagation(),this.searchPending=!0,this.requestUpdate()}searchChange(t){// console.log(`search=${this.search}`)
if(this.errorMessage="",this.errorType="",this.search=t.target.value,this.__lookupProvider){const e=this.search;this.__lookupProvider.search=e}this.requestUpdate()}doSearch(){// console.log(`doSearch:search: ${this.search}`)
this.errorMessage="",this.errorType="",this.results=[],0<this.search.length&&this.fetchFromProvider(this.search),this.requestUpdate()}loadNextPage(){// console.log(`loadNextPage`)
this.nextPageFromProvider()}loadPrevPage(){// console.log(`loadPrevPage`)
this.prevPageFromProvider()}}console.log("registering person lookup"),window.customElements.define("byu-person-lookup",be),console.log("registering person lookup results"),window.customElements.define("byu-person-lookup-results",Pe)})();
