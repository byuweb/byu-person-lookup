...
function parseAddresses (addresses) {
  if (addresses.metadata.validation_response.code >= 400) {
    return null
  }
...
function parseBasic (basic) {
  const getPreferredName = basic => {
    const lastName = lodash_get(basic, 'preferred_surname.value', '');
    const firstName = lodash_get(basic, 'preferred_first_name.value', '');
    return `${lastName}, ${firstName}`
  };
  const name = lodash_get(basic, 'name_lnf.value', getPreferredName(basic)) || getPreferredName(basic);
...
function parseEmailAddresses (emailAddresses) {
  if (emailAddresses.metadata.validation_response.code >= 400) {
    return null
  }
...
function parsePhones (phones) {
  if (phones.metadata.validation_response.code >= 400) {
    return null
  }
...
function parseEmployeeSummaries (employeeSummaries) {
  if (employeeSummaries.metadata.validation_response.code >= 400) {
    return null
  }
  return {
...
function parseStudentSummaries (studentSummaries) {
  if (studentSummaries.metadata.validation_response.code >= 400) {
    return null
  }
...
const executePersonsv2Request = async (search$$1, target, pageLink) => {
  try {
    const results = await search(search$$1, pageLink);
    if (!results) {
