 ...
 */class ByuPersonLookupResults extends LitElement{static get properties(){return{results:Array,context:String}}_render({results:e,context:t}){const n=html$1`
      <style>
        :host {
          padding: 1rem;
        }
...
`,a=e=>html$1`
    ...
    `,r=e=>/ACT/.test(e.employeeType)?html$1`
          <div>
            <div>${e.jobTitle}</div>
            <div>${e.department}</div>
            <div>${e.addresses.work?a(e.addresses.work):''}</div>
          </div>
        `:html$1`<div></div>`,[,,,,s]=faEnvelope.icon,[,,,,d]=faPhone.icon,[,,,,o]=faHome.icon,l=e=>html$1`
    ...
    `,_=e=>html$1`
      <div class="card" on-click="${()=>this.select(e)}">
        ...
        <div class="contact">
          <div>${l(s)}${e.email}</div>
          <div>${l(d)}${e.phone}</div>
          <div>${l(o)}${e.addresses.mailing?a(e.addresses.mailing):''}</div>
        </div>
      </div>
    `;if(!e||1>e.length)return html$1``;const[,,,,p]=faTimes.icon,u=svg$1`
    <path d$=${p} fill="white" transform="translate(90)"/>
    `;return html$1`
      ${n}
      <div class="modal">
        <div class="results">
    ...
    `)(e)}
          ...
          <button class="close-button" on-click="${()=>this.close()}">Close</button>
        </div>
    ...
    `}select(e){const{personId:t,byuId:n,netId:a,name:r}=e,i=new CustomEvent('byu-lookup-results-select',{detail:{personId:t,byuId:n,netId:a,name:r},bubbles:!0,composed:!0});this.dispatchEvent(i),this.dispatchEvent(new CustomEvent('byu-lookup-results-close'))}close(){this.dispatchEvent(new CustomEvent('byu-lookup-results-close'))}}console.log('registering person lookup results'),window.customElements.define('byu-person-lookup-results',ByuPersonLookupResults);export default ByuPersonLookupResults;
