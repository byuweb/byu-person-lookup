 ...
 */const directives=new WeakMap,isDirective=e=>"function"==typeof e&&directives.has(e),isCEPolyfill=window.customElements!==void 0&&window.customElements.polyfillWrapFlushCallback!==void 0,reparentNodes=(e,t,n=null,r=null)=>{for(let a=t;a!==n;){const t=a.nextSibling;e.insertBefore(a,r),a=t}},removeNodes=(e,t,n=null)=>{for(let r=t;r!==n;){const t=r.nextSibling;e.removeChild(r),r=t}},noChange={},nothing={},marker=`{{lit-${(Math.random()+"").slice(2)}}}`,nodeMarker=`<!--${marker}-->`,markerRegex=new RegExp(`${marker}|${nodeMarker}`),boundAttributeSuffix="$lit$";/**
 * An updateable Template that tracks the location of dynamic parts.
 */class Template{constructor(e,t){this.parts=[],this.element=t;let n=-1,r=0;const a=[],s=t=>{const d=t.content,i=document.createTreeWalker(d,133/* NodeFilter.SHOW_{ELEMENT|COMMENT|TEXT} */,null,!1);// Edge needs all 4 parameters present; IE11 needs 3rd parameter to be
// null
...
let s=0;for(let e=0;e<a.length;e++)0<=a[e].value.indexOf(marker)&&s++;for(;0<s--;){// Get the template literal section leading up to the first
// expression in this attribute
const a=e.strings[r],s=lastAttributeNameRegex.exec(a)[2],d=s.toLowerCase()+boundAttributeSuffix,i=t.getAttribute(d),o=i.split(markerRegex);// Find the attribute name
this.parts.push({type:"attribute",index:n,name:s,strings:o}),t.removeAttribute(d),r+=o.length-1}}"TEMPLATE"===t.tagName&&s(t)}else if(3===t.nodeType/* Node.TEXT_NODE */){const e=t.data;if(0<=e.indexOf(marker)){const s=t.parentNode,d=e.split(markerRegex),o=d.length-1;// Generate a new text node for each literal section
...
""===d[o]?(s.insertBefore(createMarker(),t),a.push(t)):t.data=d[o],r+=o}}else if(8===t.nodeType/* Node.COMMENT_NODE */)if(t.data===marker){const e=t.parentNode;// Add a new marker node to be the startNode of the Part if any of
// the following are true:
//  * We don't have a previousSibling
//  * The previousSibling is already the start of a previous part
(null===t.previousSibling||n===d)&&(n++,e.insertBefore(createMarker(),t)),d=n,this.parts.push({type:"node",index:n}),null===t.nextSibling?t.data="":(a.push(t),n--),r++}else for(let e=-1;-1!==(e=t.data.indexOf(marker,e+1));)// Comment node has a binding marker inside, make an inactive part
...
for(const r of a)r.parentNode.removeChild(r)}}const isTemplatePartActive=e=>-1!==e.index,createMarker=()=>document.createComment(""),lastAttributeNameRegex=/([ \x09\x0a\x0c\x0d])([^\0-\x1F\x7F-\x9F \x09\x0a\x0c\x0d"'>=/]+)([ \x09\x0a\x0c\x0d]*=[ \x09\x0a\x0c\x0d]*(?:[^ \x09\x0a\x0c\x0d"'`<>=]*|"[^"]*|'[^']*))$/;// Allows `document.createComment('')` to be renamed for a
...
const e=isCEPolyfill?this.template.element.content.cloneNode(!0):document.importNode(this.template.element.content,!0),t=this.template.parts;let n=0,r=0;const a=e=>{// Edge needs all 4 parameters present; IE11 needs 3rd parameter to be
// null
 ...
 * removed 4 nodes)
 */function removeNodesFromTemplate(e,t){const{element:{content:r},parts:n}=e,a=document.createTreeWalker(r,walkerNodeFilter,null,!1);let s=nextActiveIndexInTemplateParts(n),d=n[s],i=-1,o=0;const l=[];for(let r=null;a.nextNode();){i++;const e=a.currentNode;// End removal if stepped past the removing node
...
if(null===n||void 0===n)return void a.appendChild(t);const s=document.createTreeWalker(a,walkerNodeFilter,null,!1);let d=nextActiveIndexInTemplateParts(r),i=0,o=-1;for(;s.nextNode();){o++;const e=s.currentNode;for(e===n&&(i=countNodes(t),n.parentNode.insertBefore(t,n));-1!==d&&r[d].index===o;){// If we've inserted the node, simply adjust all subsequent parts
...
if(0===r.length)return void window.ShadyCSS.prepareTemplateStyles(t.element,n);const a=document.createElement("style");// Collect styles into a single style. This helps us make sure ShadyCSS
// manipulations will not prevent us from being able to fix up template
// part indices.
// NOTE: collecting styles is inefficient for browsers but ShadyCSS
// currently does this anyway. When it does not, this should be changed.
for(let s=0;s<r.length;s++){const e=r[s];e.parentNode.removeChild(e),a.textContent+=e.textContent}// Remove styles from nested templates in this scope.
if(removeStylesFromLitTemplates(n),insertNodeIntoTemplate(t,a,t.element.content.firstChild),window.ShadyCSS.prepareTemplateStyles(t.element,n),window.ShadyCSS.nativeShadow){// When in native Shadow DOM, re-add styling to rendered content using
// the style ShadyCSS produced.
const n=t.element.content.querySelector("style");e.insertBefore(n.cloneNode(!0),e.firstChild)}else{t.element.content.insertBefore(a,t.element.content.firstChild);const e=new Set;e.add(a),removeNodesFromTemplate(t,e)}},render$1=(e,t,n)=>{const r=n.scopeName,a=parts.has(t),s=t instanceof ShadowRoot&&compatibleShadyCSSVersion&&e instanceof TemplateResult,d=s&&!shadyRenderSet.has(r),i=d?document.createDocumentFragment():t;// When performing first scope render,
...
if(void 0!==e&&!this._changedProperties.has(e)){const r=this.constructor,a=r._classProperties.get(e)||defaultPropertyDeclaration;r._valueHasChanged(this[e],t,a.hasChanged)?(this._changedProperties.set(e,t),!0===a.reflect&&!(this._updateState&STATE_IS_REFLECTING_TO_PROPERTY)&&(void 0===this._reflectingProperties&&(this._reflectingProperties=new Map),this._reflectingProperties.set(e,a))):n=!1}return!this._hasRequestedUpdate&&n&&this._enqueueUpdate(),this.updateComplete}/**
...
const e=()=>[1,2,3,4,5,6].map(()=>html`
    ...
    `,s=e=>e.showAdditionalInfo&&e.additionalInfo?html`
          <ul>
    ...
    `:html``}dispatch(e,t){const n=t?{detail:t,bubbles:!0,composed:!0}:{bubbles:!0,composed:!0},r=new CustomEvent(e,n);this.dispatchEvent(r)}select(e){const{personId:t,byuId:n,netId:r,name:a}=e;this.dispatch("byu-lookup-results-select",{personId:t,byuId:n,netId:r,name:a}),this.close()}close(){this.dispatch("byu-lookup-results-close"),this.isObserving=!1}next(){this.searchPending||this.dispatch("byu-lookup-next-page")}prev(){this.searchPending||this.dispatch("byu-lookup-prev-page")}}export default ByuPersonLookupResults;
