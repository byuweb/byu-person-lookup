 ...
 */const dedupingMixin=function(e){function t(t){let r=/** @type {!MixinFunction} */t.__mixinSet;if(r&&r[a])return t;let i=n,s=i.get(t);s||(s=/** @type {!Function} */e(t),i.set(t,s));// copy inherited mixin set from the extended class, or the base class
// NOTE: we avoid use of Set here because some browser (IE11)
// cannot extend a base Set via the constructor.
let d=Object.create(/** @type {!MixinFunction} */s.__mixinSet||r||null);return d[a]=!0,s.__mixinSet=d,s}let n=/** @type {!MixinFunction} */e.__mixinApplications;n||(n=new WeakMap,e.__mixinApplications=n);// maintain a unique id for each mixin
...
let r=-1,i=0;const s=[];// The actual previous node, accounting for removals: if a node is removed
// it will never be the previousNode.
// Used to set previousNode at the top of the loop.
for(let n,d;a.nextNode();){r++,n=d;const t=d=a.currentNode;if(1===t.nodeType/* Node.ELEMENT_NODE */){if(!t.hasAttributes())continue;const n=t.attributes;// Per https://developer.mozilla.org/en-US/docs/Web/API/NamedNodeMap,
...
const a=e.strings[i],s=lastAttributeNameRegex.exec(a)[1],d=n.getNamedItem(s),o=d.value.split(markerRegex);// Find the attribute name
// Find the corresponding attribute
// TODO(justinfagnani): remove non-null assertion
this.parts.push(new TemplatePart('attribute',r,d.name,s,o)),t.removeAttribute(d.name),i+=o.length-1}}else if(3===t.nodeType/* Node.TEXT_NODE */){const e=t.nodeValue;if(0>e.indexOf(marker))continue;const n=t.parentNode,a=e.split(markerRegex),d=a.length-1;// We have a part for each match found
i+=d;// Generate a new text node for each literal section
// These nodes are also used as the markers for node parts
for(let e=0;e<d;e++)n.insertBefore(''===a[e]?document.createComment(''):document.createTextNode(a[e]),t),this.parts.push(new TemplatePart('node',r++));n.insertBefore(''===a[d]?document.createComment(''):document.createTextNode(a[d]),t),s.push(t)}else if(8===t.nodeType/* Node.COMMENT_NODE */&&t.nodeValue===marker){const e=t.parentNode,a=t.previousSibling;// Add a new marker node to be the startNode of the Part if any of the
// following are true:
...
null===a||a!==n||a.nodeType!==Node.TEXT_NODE?e.insertBefore(document.createComment(''),t):r--,this.parts.push(new TemplatePart('node',r++)),s.push(t),null===t.nextSibling?e.insertBefore(document.createComment(''),t):r--,d=n,i++}}// Remove text binding nodes after the walk to not disturb the TreeWalker
for(const a of s)a.parentNode.removeChild(a)}}/**
 ...
 */class AttributePart{constructor(e,t,n,a){this.instance=e,this.element=t,this.name=n,this.strings=a,this.size=a.length-1,this._previousValues=[]}_interpolate(e,t){const n=this.strings,a=n.length-1;let r='';for(let s=0;s<a;s++){r+=n[s];const a=getValue(this,e[t+s]);if(a&&a!==directiveValue&&(Array.isArray(a)||'string'!=typeof a&&a[Symbol.iterator]))for(const e of a)// TODO: we need to recursively call getValue into iterables...
...
`,r=()=>html$1`
    ...
    `);if(!e||!e.map||1>e.length)return html$1``;const[,,,,g]=faTimes.icon,f=svg$1`
...
n.observe(t)}}}console.log('registering person lookup results'),window.customElements.define('byu-person-lookup-results',ByuPersonLookupResults);export default ByuPersonLookupResults;
