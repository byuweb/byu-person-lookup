 ...
 */class Template{constructor(e,t){this.parts=[],this.element=t;let n=-1,a=0;const r=[],s=t=>{const d=t.content,o=document.createTreeWalker(d,133/* NodeFilter.SHOW_{ELEMENT|COMMENT|TEXT} */,null,!1);// Edge needs all 4 parameters present; IE11 needs 3rd parameter to be
// null
...
const r=e.strings[a],s=lastAttributeNameRegex.exec(r)[2],d=s.toLowerCase()+boundAttributeSuffix,o=t.getAttribute(d),i=o.split(markerRegex);// Find the attribute name
this.parts.push({type:"attribute",index:n,name:s,strings:i}),t.removeAttribute(d),a+=i.length-1}}"TEMPLATE"===t.tagName&&s(t)}else if(3===t.nodeType/* Node.TEXT_NODE */){const e=t.data;if(0<=e.indexOf(marker)){const s=t.parentNode,d=e.split(markerRegex),o=d.length-1;// Generate a new text node for each literal section
 ...
 * removed 4 nodes)
 */function removeNodesFromTemplate(e,t){const{element:{content:a},parts:n}=e,r=document.createTreeWalker(a,walkerNodeFilter,null,!1);let s=nextActiveIndexInTemplateParts(n),d=n[s],o=-1,i=0;const p=[];for(let a=null;r.nextNode();){o++;const e=r.currentNode;// End removal if stepped past the removing node
...
if(null===n||void 0===n)return void r.appendChild(t);const s=document.createTreeWalker(r,walkerNodeFilter,null,!1);let d=nextActiveIndexInTemplateParts(a),o=0,i=-1;for(;s.nextNode();){i++;const e=s.currentNode;for(e===n&&(o=countNodes(t),n.parentNode.insertBefore(t,n));-1!==d&&a[d].index===i;){// If we've inserted the node, simply adjust all subsequent parts
if(0<o){for(;-1!==d;)a[d].index+=o,d=nextActiveIndexInTemplateParts(a,d);return}d=nextActiveIndexInTemplateParts(a,d)}}}/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 * http://polymer.github.io/LICENSE.txt
...
const n=t.element.content.querySelector("style");e.insertBefore(n.cloneNode(!0),e.firstChild)}else{t.element.content.insertBefore(r,t.element.content.firstChild);const e=new Set;e.add(r),removeNodesFromTemplate(t,e)}},render$1=(e,t,n)=>{const a=n.scopeName,r=parts.has(t),s=t instanceof ShadowRoot&&compatibleShadyCSSVersion&&e instanceof TemplateResult,d=s&&!shadyRenderSet.has(a),o=d?document.createDocumentFragment():t;// When performing first scope render,
