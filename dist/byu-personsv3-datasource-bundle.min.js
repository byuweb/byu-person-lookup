 ...
 */class Template{constructor(e,t){this.parts=[],this.element=t;let n=-1,a=0;const r=[],d=t=>{const s=t.content,i=document.createTreeWalker(s,133/* NodeFilter.SHOW_{ELEMENT|COMMENT|TEXT} */,null,!1);// Edge needs all 4 parameters present; IE11 needs 3rd parameter to be
// null
...
let d=0;for(let e=0;e<r.length;e++)0<=r[e].value.indexOf(marker)&&d++;for(;0<d--;){// Get the template literal section leading up to the first
// expression in this attribute
const r=e.strings[a],d=lastAttributeNameRegex.exec(r)[2],s=d.toLowerCase()+boundAttributeSuffix,i=t.getAttribute(s),o=i.split(markerRegex);// Find the attribute name
this.parts.push({type:"attribute",index:n,name:d,strings:o}),t.removeAttribute(s),a+=o.length-1}}"TEMPLATE"===t.tagName&&d(t)}else if(3===t.nodeType/* Node.TEXT_NODE */){const e=t.data;if(0<=e.indexOf(marker)){const d=t.parentNode,s=e.split(markerRegex),o=s.length-1;// Generate a new text node for each literal section
// These nodes are also used as the markers for node parts
for(let e=0;e<o;e++)d.insertBefore(""===s[e]?createMarker():document.createTextNode(s[e]),t),this.parts.push({type:"node",index:++n});// If there's no text, we must insert a comment to mark our place.
// Else, we can trust it will stick around after cloning.
// We have a part for each match found
""===s[o]?(d.insertBefore(createMarker(),t),r.push(t)):t.data=s[o],a+=o}}else if(8===t.nodeType/* Node.COMMENT_NODE */)if(t.data===marker){const e=t.parentNode;// Add a new marker node to be the startNode of the Part if any of
// the following are true:
//  * We don't have a previousSibling
//  * The previousSibling is already the start of a previous part
(null===t.previousSibling||n===s)&&(n++,e.insertBefore(createMarker(),t)),s=n,this.parts.push({type:"node",index:n}),null===t.nextSibling?t.data="":(r.push(t),n--),a++}else for(let e=-1;-1!==(e=t.data.indexOf(marker,e+1));)// Comment node has a binding marker inside, make an inactive part
...
// node if neccessasry.
if(!isTemplatePartActive(e))this._parts.push(void 0),n++;else if(a===e.index){if("node"===e.type){const e=this.processor.handleTextExpression(this.options);e.insertAfterNode(s.previousSibling),this._parts.push(e)}else this._parts.push(...this.processor.handleAttributeExpressions(s,e.name,e.strings,this.options));n++}else a++,"TEMPLATE"===s.nodeName&&r(s.content),s=d.nextNode()}};return r(e),isCEPolyfill&&(document.adoptNode(e),customElements.upgrade(e)),e}}/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 * http://polymer.github.io/LICENSE.txt
 ...
 */class DefaultTemplateProcessor{/**
     ...
     */handleAttributeExpressions(e,t,n,a){const r=t[0];if("."===r){const a=new PropertyCommitter(e,t.slice(1),n);return a.parts}if("@"===r)return[new EventPart(e,t.slice(1),a.eventContext)];if("?"===r)return[new BooleanAttributePart(e,t.slice(1),n)];const d=new AttributeCommitter(e,t,n);return d.parts}/**
 ...
 * removed 4 nodes)
 */function removeNodesFromTemplate(e,t){const{element:{content:a},parts:n}=e,r=document.createTreeWalker(a,walkerNodeFilter,null,!1);let d=nextActiveIndexInTemplateParts(n),s=n[d],i=-1,o=0;const l=[];for(let a=null;r.nextNode();){i++;const e=r.currentNode;// End removal if stepped past the removing node
...
if(null===n||void 0===n)return void r.appendChild(t);const d=document.createTreeWalker(r,walkerNodeFilter,null,!1);let s=nextActiveIndexInTemplateParts(a),i=0,o=-1;for(;d.nextNode();){o++;const e=d.currentNode;for(e===n&&(i=countNodes(t),n.parentNode.insertBefore(t,n));-1!==s&&a[s].index===o;){// If we've inserted the node, simply adjust all subsequent parts
if(0<i){for(;-1!==s;)a[s].index+=i,s=nextActiveIndexInTemplateParts(a,s);return}s=nextActiveIndexInTemplateParts(a,s)}}}/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 * http://polymer.github.io/LICENSE.txt
...
const n=t.element.content.querySelector("style");e.insertBefore(n.cloneNode(!0),e.firstChild)}else{t.element.content.insertBefore(r,t.element.content.firstChild);const e=new Set;e.add(r),removeNodesFromTemplate(t,e)}},render$1=(e,t,n)=>{const a=n.scopeName,r=parts.has(t),d=t instanceof ShadowRoot&&compatibleShadyCSSVersion&&e instanceof TemplateResult,s=d&&!shadyRenderSet.has(a),i=s?document.createDocumentFragment():t;// When performing first scope render,
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const{CustomEvent:CustomEvent$1}=window,executePersonsv3Request=async(e,t,n)=>{try{const{next:a,prev:r,people:d}=await search(e,n);return t.dispatchEvent(new CustomEvent$1("byu-lookup-datasource-result",{bubbles:!0,detail:d})),{next:a,prev:r}}catch(e){console.error(e),t.dispatchEvent(new CustomEvent$1("byu-lookup-datasource-error",{bubbles:!0,detail:e}))}},setPendingSearch=e=>{const t=new CustomEvent$1("byu-lookup-datasource-searching",{bubbles:!0});e.dispatchEvent(t)};class ByuPersonsv3Datasource extends LitElement{connectedCallback(){super.connectedCallback(),connect();const e=new CustomEvent$1("byu-lookup-datasource-register",{bubbles:!0});this.dispatchEvent(e)}disconnectedCallback(){super.disconnectedCallback(),disconnect()}static get properties(){return{search:{type:String},next:{type:String},prev:{type:String}}}render(){this.search||(this.search="");const{label:e}=resolveSearchType(this.search);return html`${e}`}async performSearch(e){this.timeout&&clearTimeout(this.timeout),this.timeout=setTimeout(async()=>{setPendingSearch(this);const{next:e,prev:t}=await executePersonsv3Request(this.search,this);this.next=e,this.prev=t},100)}async nextPage(){this.timeout&&clearTimeout(this.timeout),this.timeout=setTimeout(async()=>{if(this.next){setPendingSearch(this);const{next:e,prev:t}=await executePersonsv3Request(this.search,this,this.next);this.next=e,this.prev=t}},100)}async prevPage(){this.timeout&&clearTimeout(this.timeout),this.timeout=setTimeout(async()=>{if(this.prev){setPendingSearch(this);const{next:e,prev:t}=await executePersonsv3Request(this.search,this,this.prev);this.next=e,this.prev=t}},100)}}console.log("registering personsv3 datasource"),window.customElements.define("byu-personsv3-datasource",ByuPersonsv3Datasource);
